...
class PointsController {
    async index(request: Request, response: Response) {
        ...
        const points = await knex("points")
            ...
            .distinct()
            .select("points.*")
        const serializedPoints = points.map(point => ({ ...point, image_url: `http://192.168.1.102:3333/uploads/${point.image}` }))
    ...
    async show(request: Request, response: Response) {
        ...
        const serializedPoint = { ...point, image_url: `http://192.168.1.102:3333/uploads/${point.image}` }
